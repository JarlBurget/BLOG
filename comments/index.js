...
app.post("/posts/:id/comments", (req, res) => {
  try {
    ...
    // Only mark as pending if comment contains banned words; default to approved
    const bannedWords = ["orange"];
    const containsBanned = bannedWords.some((w) =>
      commentContent.toLowerCase().includes(w.toLowerCase())
    );
    ...
    const comment = {
      ...
      status: containsBanned ? "pending" : "approved",
    };
